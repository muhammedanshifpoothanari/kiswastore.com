...
export default function Slider() {
  ...
  useEffect(() => {
    // Load video only after page is interactive + element is in viewport
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // Small delay so critical page assets load first
          setTimeout(() => setLoadVideo(true), 800)
          observer.disconnect()
        }
      },
      { threshold: 0.1 }
  ...
  return (
    ...
    >
      {/* Fallback image — visible instantly, zero delay */}
      <Image
      ...
      {/* Vimeo — only injected after page is ready */}
      {loadVideo && (
